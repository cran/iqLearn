...
\noindent {\em Keywords:} Interactive Q-learning; Q-learning; Dynamic Treatment Regimes; Dynamic Programming; SMART design.
...
$\pi_{t}^{\hbox{\scriptsize IQ-opt}}$ and the Q-learning analogs by $\pi_{t}^{\hbox{\scriptsize Q-opt}}$.
...
$Q_{t}$-function, 
\begin{equation*}
  \widehat{\pi}_{t}^{\hbox{\scriptsize Q-opt}}(\bh_{t}) = \arg\max_{a_{t}}
...
\newpage  
  ...
  \begin{center}
    \begin{tabular}{|l p{3.3in} |}
      ...
      $\{\bH_{10,i}, \bH_{11,i}, A_{1,i}\}_{i=1}^{n}$ to obtain an estimator $\widehat{\ell}(\bH_{1},
